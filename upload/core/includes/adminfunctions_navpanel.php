...
// (depending on value of $cpnavjs)
	...
	if (VB_AREA == 'AdminCP')
	{
		...
		{
			$dowhat = 'collapse';
			$imagefile = 'cp_' . $dowhat . '.' . $vb_options['cpstyleimageext'];
			$style = '';
		...
		{
			$dowhat = 'expand';
			$imagefile = 'cp_' . $dowhat . '_' . vB_Template_Runtime::fetchStyleVar('textdirection') . '.' . $vb_options['cpstyleimageext'];
			$style = 'display:none';
		...
		<tr>
			...
			<td align=\"" . vB_Template_Runtime::fetchStyleVar('right') . "\">
				...
				><img class=\"acp-nav-arrow\" src=\"" . get_cpstyle_href($imagefile) . "\" title=\"$tooltip\" id=\"button_{$nav_file}_$groupid[$nav_file]\" alt=\"+\" border=\"0\" /></a
			</td>
		</tr>
		</table>";
		$_NAV .= "
...
?>
	...
	function open_close_group(group, doOpen)
	{
		...
		if (doOpen)
		{
			...
			curbtn.src = "<?php echo get_cpstyle_href('cp_collapse.' . $options['cpstyleimageext']); ?>";
		...
		else
		{
			...
			curbtn.src = "<?php echo get_cpstyle_href('cp_expand_' . vB_Template_Runtime::fetchStyleVar('textdirection') . '.' . $options['cpstyleimageext']); ?>";
