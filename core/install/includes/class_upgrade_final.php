...
class vB_Upgrade_final extends vB_Upgrade_Version
{
	...
	function step_3()
	{
		...
		if (is_dir($customlangdir))
		{
			if ($dh = opendir($customlangdir))
			{
				...
				while (($file = readdir($dh)) !== false)
				{
					if (strpos($file, 'vbulletin-custom-language-') === 0)
					{
						...
						else
						{
							...
							$langid = xml_import_language($xml, 0, '', true, true, !defined('SUPPRESS_KEEPALIVE_ECHO'), true);
							if ($langid === false)
							{
							...
							else
							{
								...
								if (VB_AREA == 'Install')
								{
									$vblangcode = $assertor->getColumn('language', 'vblangcode', array('languageid' => $langid));
				...
				$set = $db->select(
					'language',
					array('languageid' => $languagesLoaded),
					false,
				...
				foreach($set AS $row)
				{
					$type = 'vbulletin-custom-language-' . $row['vblangcode'];
					$xmlfiles = $productlib->loadProductXmlList($type);

					foreach($xmlfiles AS $file)
					{
						if (!($xml = file_read($file)))
						{
							$this->add_error(sprintf($this->phrase['vbphrase']['file_not_found'], $file), self::PHP_TRIGGER_ERROR, false);
						}
						else
						{
							$result = xml_import_language($xml, $row['languageid'], '', true, true, !defined('SUPPRESS_KEEPALIVE_ECHO'), true);
						}
					}
				}
	...
	function step_13($data = null)
	{
		...
		else
		{
			...
			do_rebuilds_after_product_install($rebuild, $rebuildAll);
