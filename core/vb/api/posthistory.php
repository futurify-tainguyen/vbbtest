	...
	public function fetchHistoryComparison($nodeid, $oldpost, $newpost)
	{
		...
		foreach ($diff AS $diffrow)
		{
			...
			if ($diffrow->old_class == 'unchanged' AND $diffrow->new_class == 'unchanged')
			{
				// no change
				$results[] = array(
					'unchanged_olddata' => nl2br(vB_String::htmlSpecialCharsUni(implode("\n", $diffrow->fetch_data_old())))
				);
			}
			else
			{
				...
				$results[] = array(
					'changed_olddata' => nl2br(vB_String::htmlSpecialCharsUni(implode("\n", $diffrow->fetch_data_old()))),
					'changed_newdata' => nl2br(vB_String::htmlSpecialCharsUni(implode("\n", $diffrow->fetch_data_new()))),
				);
			}
		}
