...
class vB_Library_Node extends vB_Library
{
	...
	// Cached result of vB_Node::getExcludedTypes(), used by fixNodeLast().
	// Cached because we use these multiple times in a loop.
	protected $excludedTypes;
	// Same as above, but without vBForum_Channel's contenttypeid, currently
	// used exclusively by fixNodeLast()
	protected $excludedTypesIncludeChannel;
	...
	public function listNodesInternal($parentid, $page, $perpage, $depth, $contenttypeid, $options, $withJoinableContent = false)
	{
		...
		//my_channels
		if (!empty($options['my_channels']) AND in_array($options['my_channels'], array('group', 'blog'), true))
		{
			$criteria['my_channels'] = array('type' => $options['my_channels']);
		}
	...
	public function fetchNodeAttachments($parentids)
	{
		...
		if (!empty($notfound))
		{
			...
			try
			{
				$attachments = vB::getDbAssertor()->getRows(
	...
	*/
	public function markRead($nodeid)
	{
		...
		$userid = vB::getCurrentSession()->get('userid');
		$threadMarking = $datastore->getOption('threadmarking');
		...
		if ($threadMarking == 2)
		{
			...
			{
				$result = $db->getRow('vBForum:getChannelRead', array(
					'channelid' => $parentid,
					'userid' => $userid,
					'channelcontenttypeid' => $this->channelTypeId,
		...
		$db->assertQuery('vBForum:nodeMarkread', array('nodeid' => $nodes_marked, 'userid' => $userid, 'readtime' => $timenow));
		return $nodes_marked;
	}
	...
	*/
	...
	public function undeleteNodes($nodeids, $needRebuild = false)
	{
		...
		foreach ($nodeids AS $nodeid)
		{
			...
			foreach ($parents as $parent)
			{
				...
				{
					// TODO: there's still one more usage of vBForum:updateLastData. When we get to refactoring lastcontent changes,
					// unify the lastcontent detection/updates & remove (if unnecessary) the query & references to it.
					//$result = $assertor->assertQuery('vBForum:updateLastData', array('parentid' => $parent['parent'], 'timenow' => vB::getRequest()->getTimeNow()));
					// note, updateLastForSubtree() returns null
					$result = $this->updateLastForSubtree($parent['parent']);
				}
	...
	private function initExcludedTypes()
	{
		// Cannot initialize these in the constructor ATM due to circular references.
		$this->excludedTypes = vB_Node::getExcludedTypes();
		$this->excludedTypesIncludeChannel = $this->excludedTypes;
		foreach ($this->excludedTypesIncludeChannel AS $__key => $__id)
		{
			if ($this->channelTypeId == $__id)
			{
				unset($this->excludedTypesIncludeChannel[$__key]);
			}
		}
	}
		...
		$result = $db->assertQuery('vBForum:node',
			array(
				...
				vB_dB_Query::CONDITIONS_KEY => array(
					...
					'showpublished' => 1,
					'showapproved' => 1,
					// todo: 'inlist' => 1? only photo & attach have inlist = 0,
					// and adding inlist = 1 filter would allow us to expand the parent_lastcontent index to add inlist before
					// lastcontent, lastcontentid and allow usage of that index for topics-fetch queries
					array('field' => 'contenttypeid', 'value' => $this->excludedTypesIncludeChannel, 'operator' => vB_dB_Query::OPERATOR_NE),
				),
				vB_dB_Query::PARAM_LIMIT => 1,
