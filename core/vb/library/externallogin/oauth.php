...
class vB_Library_ExternalLogin_OAuth extends vB_Library_ExternalLogin
{
	...
	*/
	...
	protected function doPOSTRequest($url, $parameters)
	{
		...
		$vurl->set_option(VURL_POSTFIELDS, $urlEncodedPostData);
		...
		// If the third party redirects us for some reason, I'm not sure if we want to just follow it
	...
	protected function doGETRequest($url, $parameters)
	{
		...
		$vurl = new vB_vURL();
		$vurl->set_option(VURL_URL, $url . "?" . $urlEncodedPostData);
		$vurl->set_option(VURL_USERAGENT, 'vBulletin/' . SIMPLE_VERSION);
		$vurl->set_option(VURL_POST, 0);
		$vurl->set_option(VURL_HTTPHEADER, $httpHeaders);
		$vurl->set_option(VURL_RETURNTRANSFER, 1); // We want to examine the return, not output it directly.
		$vurl->set_option(VURL_HEADER, 1); // We want the headers to examine as well.
		$vurl->set_option(VURL_TIMEOUT, 5); // Timeout of 5s
