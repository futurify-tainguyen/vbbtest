...
class vB_ChannelPermission
{
	...
	protected static $permissionFields = array(
		...
		//'require_moderate'     => self::TYPE_BOOL, // replaced by skip_moderate, see VBV-12380 & VBV-11909
		'skip_moderate'        => self::TYPE_BOOL,
		...
		foreach (self::$permissionFields AS $fieldname => $fieldtype)
		{
			...
			switch ($fieldtype)
			{
				...
				case self::TYPE_HOURS:
					$params[$fieldname] = floatval($permissions[$fieldname]);
					break;
				case self::TYPE_BITMAP :
					if (is_numeric($permissions[$fieldname]))
					{
