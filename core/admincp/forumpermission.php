...
define('CVS_REVISION', '$RCSfile$ - $Revision: 101130 $');
	...
	foreach ($vbulletin->GPC['usergrouplist'] AS $ugid_to => $confirm)
	{
		...
		while($result AND $result->valid())
		{
			...
			$assertor->assertQuery('replacePermissions', array(
				...
				'skip_moderate' => $permission['skip_moderate'],
...
if ($_POST['do'] == 'doquickforum')
{
	$vbulletin->input->clean_array_gpc('p', array(
		...
		'skip_moderate'	    => vB_Cleaner::TYPE_INT,
	...
	foreach ($vbulletin->GPC['usergrouplist'] AS $usergroupid => $confirm)
	{
		if ($confirm == 1)
		{
			...
			$permissions =  array(
				...
				'skip_moderate' => $vbulletin->GPC['skip_moderate'],
			...
			vB_ChannelPermission::instance()->setPermissions($vbulletin->GPC['nodeid'], $usergroupid, $permissions);
		}
	}
