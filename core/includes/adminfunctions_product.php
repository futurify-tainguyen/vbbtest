...
function delete_product($productid, $compliant_35 = false, $for_product_upgrade = false)
{
	...
	if (is_newer_version($options['templateversion'], '5.0.0 Alpha 1', true))
	{
		...
		if (!$for_product_upgrade)
		{
			$widgetids = $assertor->getColumn('widget', 'widgetid', array('product' => $productid));
			$assertor->delete('widget', array('product' => $productid));
			$assertor->delete('widgetdefinition', array('product' => $productid));
			if(!empty($widgetids))
			{
				$instanceids = $assertor->getColumn('widgetinstance', 'widgetinstanceid', array('widgetid' => $widgetids));
...
function install_product($xml, $allow_overwrite = false, $verbose = true, $deferRebuild = false)
{
	...
	if (isset($arr['templates']['template']) AND is_array($arr['templates']['template']))
	{
		...
		foreach ($templates AS $template)
		{
			...
			else
			{
				...
				//only store for actual templates;
				$template_titles[] = $title;
			}

			if (++$querytemplates % 20 == 0)
			{
				save_template_bunch($assertor, $querybits, $options['cache_templates_as_files'], $template_titles);
				$querybits = array();
				$template_titles = array();
			}
	...
	if (isset($arr['options']['settinggroup']) AND is_array($arr['options']['settinggroup']))
	{
		...
		foreach ($settinggroups AS $group)
		{
			...
			{
				if (isset($options["$setting[varname]"]))
				{
					$newvalue = $options["$setting[varname]"];
				}
				else
				{
	...
	if (!$deferRebuild)
	{
		do_rebuilds_after_product_install($rebuild);
	}
...
function do_rebuilds_after_product_install($rebuild, $rebuildAll = false)
{
...
function save_template_bunch($assertor, $querybits, $save_as_files, $template_names)
{
	$assertor->assertQuery('replaceValues', array('values' => $querybits, 'table' => 'template'));

	if($save_as_files)
	{
		$result = $assertor->select('template', array('styleid' => -1, 'title' => $template_names), false, array('templateid'));
		$templateids = array();
